...
*   **Dynamic Model Discovery:** To solve the 429 quota errors, I attempted to migrate to `gemini-1.5-flash` for its generous free tier, but the API returned a `404 NOT_FOUND` error. Instead of guessing model names, I wrote a custom utility script (`src/check_models.py`) to query the Google API directly via `client.models.list()`. I discovered that the `1.5` family is deprecated for my endpoint in late 2026, but `gemini-2.5-flash` is actively supported. I dynamically repinned the pipeline to `gemini-2.5-flash`, effectively bypassing the strict "Pro" model rate limits while maintaining high extraction speed.
## 4. With 2 More Weeks...
