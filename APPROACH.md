...
*   **Template Formatting Loss:** Initially, I used `pandas.to_excel()` to export the mapped data. However, I noticed this flattened the workbook and destroyed the official template's color-coded headers (Green/Amber/Grey) and multi-row hierarchy. 
    *   **Resolution:** I refactored the export logic to use `openpyxl.load_workbook()`. This allowed me to load the underlying XML of the original template, preserve the instruction sheet and color-coded headers (Rows 1-4), and seamlessly append the extracted BoQ items starting at Row 5 without corrupting the required schema.
